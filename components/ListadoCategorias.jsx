    ...
    return (
        <div className='flex flex-col mt-10 bg-white py-5 px-10 rounded-3xl gap-5'>
            {categorias?.length > 0 ? categorias.map(categoria => (
                <div key={categoria.id} className='flex gap-5 items-center justify-between border px-5 h-36'>
                    <p className='w-1/12'>{categoria.id}</p>
                    <p className='w-4/12'>{categoria.nombre}</p>
                    <div className='flex items-center justify-center w-4/12'>
                        <Image src={categoria.icono} width={80} height={80} alt={`Imagen Categoria ${categoria.nombre}`} className='h-full' />
                    </div>
                    <div className='flex gap-5 w-3/12 justify-center'>
